/*=============================================================================
 Nombre del archivo : JwtAuthenticationFilterEmpresaReadDetalleTest.java
 Descripcion        : Prueba de carga del permiso dinamico para consultar el detalle de empresa.
===============================================================================
 CONTROL DE CAMBIOS
 +------------+---------+----------------------+------------------------------------------------------------------------------------------------------------------------------------+
 |   Fecha    | Version |      Autor           | Descripcion del cambio                                                                                                             |
 +------------+---------+----------------------+------------------------------------------------------------------------------------------------------------------------------------+
 | 2026-07-27 | 1.0.0   | JUAN DIAZ            | Creacion de prueba para comprobar EMPRESA_READ requerido por la HU-043.3.                                                         |
 +------------+---------+----------------------+------------------------------------------------------------------------------------------------------------------------------------+
=============================================================================*/
...
import com.inventario.infrastructure.security.service.DynamicRolePermissionService;
...
class JwtAuthenticationFilterEmpresaReadDetalleTest {
	...
	@Test
	void doFilter_cargaAutoridadEmpresaRead() throws Exception {
		...
		when(claims.getSubject()).thenReturn("usuario@inmero.co");
		...
		when(userRepository.findTokenVersionByUsername("usuario@inmero.co")).thenReturn(Optional.of(3));
		when(jwtUtil.isTokenExpired("token-valido")).thenReturn(false);
		Collection<GrantedAuthority> authorities = List.of(
				new SimpleGrantedAuthority("ROL_EMPRESA"),
				new SimpleGrantedAuthority("EMPRESA_READ"));
		...
		assertTrue(SecurityContextHolder.getContext()
			...
			.getAuthorities()
			.stream()
			.anyMatch(authority -> "EMPRESA_READ".equals(authority.getAuthority())));
	}
