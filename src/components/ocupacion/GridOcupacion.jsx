// src/components/Ocupacion/GridOcupacion.jsx
...
export default function GridOcupacion({
  ...
  const columns = useMemo(() => ([
    ...
    // Tipo Actividad: nombre si viene anidado; fallback al campo plano o ID
    {
      field: "tipoActividadNombre",
      headerName: "Tipo Actividad",
      width: 240,
      valueGetter: (p) =>
        p?.row?.tipoActividadNombre ??
        p?.row?.tipoActividad?.nombre ??
        p?.row?.tipoActividad?.name ??
        String(p?.row?.tipoActividadId ?? ""),
    },

    // Evaluación: nombre si viene anidado; fallback al ID
    {
      field: "evaluacionId",
      headerName: "Evaluación",
      width: 200,
      valueGetter: (p) =>
        p?.row?.evaluacion?.nombre ??
        p?.row?.evaluacion?.name ??
        String(p?.row?.evaluacionId ?? ""),
    },
...
GridOcupacion.propTypes = {
  ocupaciones: PropTypes.array,
