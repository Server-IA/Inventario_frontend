...
===============================================================================
 ...
 +------------+---------+----------------------+------------------------------------------------------------------------------------------------------------------------------------+
=============================================================================*/
package com.inventario.exceptionHandler;
 ...
 * errores específicos y estandarizando las
 * respuestas HTTP.
 * <p>
 * Esta clase utiliza la anotación {@link RestControllerAdvice} para aplicar
 * lógica transversal de manejo de errores en
 * todos los controladores de la aplicación. Extiende de
 * {@link ResponseEntityExceptionHandler} para heredar y
 ...
 * estructuras de respuesta consistentes (RFC 7807
 * Problem Details), facilitando al cliente el consumo y entendimiento de los
 * errores de entrada.
 * </p>
 *
 * @author jujcgu
...
public class Advice extends ResponseEntityExceptionHandler {
     ...
     * identificar problemas de validación en la
     * plataforma Inventario. En caso de múltiples errores en un mismo campo, los
     * mensajes se concatenan separados por
     * punto y coma.
     * </p>
     *
     ...
     *         respuesta con los detalles del
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
        ...
        problemDetail.setType(URI.create("https://inmero.co/inventario/errors/validation"));
     ...
     * técnico en una respuesta estructurada
     * (RFC 7807). Mapea el incidente al código de estado HTTP
     * <code>401 UNAUTHORIZED</code> y define un mensaje
     * amigable ("El token de sesión ha caducado...") para guiar al usuario final
     ...
     *         "Token Expirado" y las instrucciones de
     *         remediación.
    ...
    public ProblemDetail handleExpiredJwtException(ExpiredJwtException ex) {
        ...
        problemDetail.setType(URI.create("https://inmero.co/inventario/errors/jwt-expired"));
    ...
    /**
     * Captura las excepciones de acceso denegado y construye una respuesta de error
     * estructurada.
     * <p>
     * Este método se invoca cuando un usuario autenticado intenta acceder a un
     * recurso para el cual no posee los
     * permisos suficientes (Autorización). Utiliza el esquema {@link ProblemDetail}
     * para estandarizar la respuesta con
     * un estado HTTP 403 (Forbidden), incluyendo un mensaje localizado basado en el
     * {@link Locale} de la petición.
     * </p>
     *
     * @param ex     La excepción {@link AccessDeniedException} lanzada por el marco
     *               de seguridad.
     * @param locale La configuración regional actual, utilizada para resolver el
     *               mensaje de error traducido.
     * @return Una instancia de {@link ProblemDetail} que encapsula los detalles del
     *         error, el título y el tipo de
    ...
    public ProblemDetail handleAccessDeniedException(AccessDeniedException ex, Locale locale) {
        ...
        problemDetail.setType(URI.create("https://inmero.co/inventario/errors/forbidden"));
    ...
    /**
     * Captura y procesa las excepciones de autenticación para generar una respuesta
     * de error estructurada.
     * <p>
     * Este método intercepta cualquier {@link AuthenticationException} lanzada
     * durante el procesamiento de la petición.
     * Utiliza el {@code MessageSource} disponible para resolver un mensaje de error
     * localizado según el {@link Locale}
     * del usuario. La respuesta se encapsula en un objeto {@link ProblemDetail} con
     * el estado HTTP 401.
     * </p>
     *
     * @param ex     La excepción de autenticación capturada que contiene los
     *               detalles del fallo.
     * @param locale La configuración regional de la petición actual, utilizada para
     *               la internacionalización del
     *               mensaje.
     * @return Una instancia de {@link ProblemDetail} que contiene el estado
     *         {@code UNAUTHORIZED}, el título y el
     *         mensaje descriptivo.
     * @see ProblemDetail
     */
    ...
    public ProblemDetail handleAuthenticationException(AuthenticationException ex, Locale locale) {
        ...
        problemDetail.setType(URI.create("https://inmero.co/inventario/errors/unauthorized"));

        return problemDetail;
    }

    /**
     * Personaliza el manejo de excepciones de lectura de mensajes HTTP para refinar
     * los detalles de errores de formato.
     * <p>
     * Sobrescribe el comportamiento base para interceptar errores de
     * deserialización JSON. Aunque delega la creación
     * inicial de la respuesta a la clase padre, este método inspecciona la causa
     * raíz ({@link Throwable}) en busca de
     * excepciones de tipo {@link InvalidFormatException}.
     * </p>
     * <p>
     * Específicamente, si el error se origina al intentar deserializar un campo de
     * tipo {@link Boolean}, se inyecta un
     * mensaje de detalle dinámico en el objeto {@link ProblemDetail} resultante,
     * indicando el valor inválido recibido y
     * aclarando que se espera un booleano estricto.
     * </p>
     *
     * @param ex      La excepción {@link HttpMessageNotReadableException}
     *                capturada, que envuelve el error de
     *                deserialización subyacente.
     * @param headers Los encabezados HTTP que se aplicarán a la respuesta.
     * @param status  El código de estado HTTP seleccionado para este error
     *                (generalmente 400 Bad Request).
     * @param request La solicitud web actual durante la cual ocurrió la excepción.
     * @return Un objeto {@link ResponseEntity} que contiene el cuerpo
     *         {@link ProblemDetail} enriquecido con mensajes
     *         específicos de tipo.
    ...
    /**
     * Utility method to dig through the exception chain and find the root cause.
     */
    private Throwable getRootCause(Throwable throwable) {
        // If it's a Spring NestedRuntimeException, we can use its built-in method
        if (throwable instanceof NestedRuntimeException nested) {
    ...
    public ProblemDetail handleDataIntegrityViolation(DataIntegrityViolationException ex, Locale locale) {
        ...
        problemDetail.setType(URI.create("https://inmero.co/inventario/errors/data-integrity"));
