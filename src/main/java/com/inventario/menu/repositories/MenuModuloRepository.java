package com.inventario.menu.repositories;
    ...
    WHERE me.moe_empresa_id = :empresaId
      ...
      AND EXISTS (
          ...
          WHERE p.modulo_id = m.mod_id
            AND p.estado_id = 1
            AND rp.estado_id = 1
            /* Casteo explícito a boolean para evitar ambigüedades en el driver JDBC de PostgreSQL */
            AND (CAST(:filtrarAdminEmpresa AS boolean) = false OR p.admin_empresa = true)
            AND er.empresa_id = :empresaId
            AND er.rol_id = :rolId
      )
    ORDER BY s.sub_nombre ASC, m.mod_nombre ASC
""", nativeQuery = true)
List<SubModuloRow> findSubmodulosByEmpresaTipoAppAndRolId(
        @Param("empresaId") Long empresaId,
        @Param("tipoAppId") Integer tipoAppId,
