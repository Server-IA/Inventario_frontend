...
/**
 * <h5>Servicio para ADMINISTRADOR_EMPRESA</h5> Gestiona la asignación de permisos a roles por módulo.
 */
...
public class RolPermisoService {
    ...
    /**
     * Obtiene módulos disponibles con sus permisos agrupados, con paginación. Optimizado para evitar cargar todos los
     * permisos en memoria.
     * 
    ...
    public Page<ModuloPermisoResponse> getModulosDisponibles(Pageable pageable) {
        ...
        // 2. Traer TODOS los permisos, módulos y métodos de esos IDs (1 consulta gigante y optimizada)
        ...
        return moduloIdsPage.map(moduloId -> {
            ...
            List<ModuloPermisoResponse.PermisoDTO> permisosDTO = permisosModulo.stream()
                    .map(p -> new ModuloPermisoResponse.PermisoDTO(p.getId(), p.getNombre(), p.getAutoridad(),
                            p.getMetodo() != null ? p.getMetodo().getNombre() : null, // Sin N+1
                            p.getUri()))
                    .toList();

            return ModuloPermisoResponse.builder().moduloId(moduloId)
                    .moduloNombre(primerPermiso.getModulo().getNombre()).moduloUrl(primerPermiso.getModulo().getUrl())
                    .moduloDescripcion(primerPermiso.getModulo().getDescripcion())
                    .moduloIcon(primerPermiso.getModulo().getIcon()).permisos(permisosDTO).build();
        });
    }

    /**
     * Obtiene módulos agrupados con permisos filtrando por uno o varios ids de subsistema. Devuelve lista sin
     * paginación para selección UI a nivel de subsistema. Útil para que el admin de empresa seleccione subsistemas y
     * luego vea todos sus módulos para elegir permisos individuales.
     */
    @Transactional(readOnly = true)
    ...
    public RolPermisoAsignadoResponse asignarModulosPermisos(Long rolId, List<Long> modulosIds) {
        ...
        return RolPermisoAsignadoResponse.builder().rolId(empresaRol.getRol().getId())
                .rolNombre(empresaRol.getRol().getNombre()).permisosAsignados(permisos.size()).modulos(modulos)
                .autoridades(autoridades).build();
    }

    /**
     * Asigna SOLO los permisos de lectura (método GET o autoridad con 'READ') de los módulos seleccionados a un rol.
     */
    @Transactional
    ...
    /**
     * Reemplaza un permiso individual por otro en la asignación de un rol-empresa. Caso de uso: "asigné el permiso X
    ...
    public RolPermisoAsignadoResponse reemplazarPermisoDeEmpresaRol(Long rolId, Long permisoIdActual,
        ...
        RolPermiso rolPermiso = RolPermiso.builder().empresaRol(empresaRol).permiso(nuevoPermiso).estado(estadoActivo)
                .createdBy(currentUser).createdAt(OffsetDateTime.now()).build();
        rolPermisoRepository.save(rolPermiso);

        return RolPermisoAsignadoResponse.builder().rolId(empresaRol.getRol().getId())
                .rolNombre(empresaRol.getRol().getNombre()).permisosAsignados(1)
                .modulos(List.of(nuevoPermiso.getModulo().getNombre()))
                .autoridades(List.of(nuevoPermiso.getAutoridad())).build();
    }

    /**
     * Reemplaza todos los permisos de un módulo por los permisos de otro módulo. Caso de uso: "asigné el módulo X por
     * error, quiero reemplazarlo completamente por el módulo Y"
     * 
     * @param rolId ID del rol
    ...
    public RolPermisoAsignadoResponse reemplazarModuloPermisosDeEmpresaRol(Long rolId, Long moduloIdActual,
        ...
        return RolPermisoAsignadoResponse.builder().rolId(empresaRol.getRol().getId())
                .rolNombre(empresaRol.getRol().getNombre()).permisosAsignados(permisosAsignados).modulos(modulos)
                .autoridades(autoridades).build();
    }
    ...
    /**
     * Asigna permisos de módulos seleccionados filtrando por los métodos indicados por módulo. Cada entrada indica un
     * módulo y la lista de métodos a asignar (o "ALL").
     */
    @Transactional
    public RolPermisoAsignadoResponse asignarModulosPermisosConMetodos(Long rolId,
        ...
        for (ModuloMetodoRequest mm : modulosMetodos) {
            ...
            Set<String> métodos = mm.getMetodos().stream().map(m -> m == null ? "" : m.trim().toUpperCase())
        ...
        for (Permiso permiso : permisosToAssign) {
            if (!rolPermisoRepository.existsByEmpresaRolIdAndPermisoId(empresaRol.getId(), permiso.getId())) {
                RolPermiso rp = RolPermiso.builder().empresaRol(empresaRol).permiso(permiso).estado(estadoActivo)
                        .createdBy(currentUser).createdAt(OffsetDateTime.now()).build();
                rolPermisoRepository.save(rp);
            }
        }

        List<String> modulos = permisosToAssign.stream().map(p -> p.getModulo().getNombre()).distinct()
                .collect(Collectors.toList());

        List<String> autoridades = permisosToAssign.stream().map(Permiso::getAutoridad).collect(Collectors.toList());

        return RolPermisoAsignadoResponse.builder().rolId(empresaRol.getRol().getId())
                .rolNombre(empresaRol.getRol().getNombre()).permisosAsignados(permisosToAssign.size()).modulos(modulos)
                .autoridades(autoridades).build();
    }
}
