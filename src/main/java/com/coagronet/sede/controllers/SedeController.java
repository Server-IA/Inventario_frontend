...
public class SedeController {
	...
	public SedeController(SedeRepository sedeRepository, SedeMapper sedeMapper, EstadoRepository estadoRepository) {
	...
	private ResponseEntity<SedeDTO> findById(@PathVariable Long requestedId) {
		...
		return sedeRepository.findByIdAndEmpresaIdAndEstadoIdNot(requestedId, empresa.getId(), 2).map(sedeMapper::toDto)
	...
	private ResponseEntity<Void> createSede(@RequestBody SedeDTO newSedeRequest, UriComponentsBuilder ucb) {
		...
		URI locationOfNewSede = ucb.path("/api/v1/sede/{id}").buildAndExpand(sede.getId()).toUri();
	...
	private ResponseEntity<List<SedeDTO>> findAll() {
		User authenticatedUser = getAuthenticatedUser();
		Empresa empresa = getEmpresaFromUser(authenticatedUser);
		List<SedeDTO> sedeDTOs = sedeRepository.findByEmpresaIdAndEstadoIdNot(empresa.getId(), 2).stream()
				.map(sedeMapper::toDto).collect(Collectors.toList());

		return ResponseEntity.ok(sedeDTOs);
	}

	@PutMapping("/{requestedId}")
	private ResponseEntity<Void> putSede(@PathVariable Long requestedId, @RequestBody SedeDTO sedeUpdate) {
		User authenticatedUser = getAuthenticatedUser();
		Empresa empresa = getEmpresaFromUser(authenticatedUser);
	...
	private ResponseEntity<List<SedeMinimalDTO>> findAllMinimal() {
		User authenticatedUser = getAuthenticatedUser();
		Empresa empresa = getEmpresaFromUser(authenticatedUser);
		List<SedeMinimalDTO> sedeMinimalDTOs = sedeRepository.findByEmpresaIdAndEstadoIdNot(empresa.getId(), 2).stream()
				.map(sedeMapper::toMinimalDTO).collect(Collectors.toList());
	...
	private ResponseEntity<SedeMinimalDTO> findMinimalById(@PathVariable Long requestedId) {
		User authenticatedUser = getAuthenticatedUser();
		Empresa empresa = getEmpresaFromUser(authenticatedUser);
		return sedeRepository.findByIdAndEmpresaIdAndEstadoIdNot(requestedId, empresa.getId(), 2)
				.map(sedeMapper::toMinimalDTO).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
	}
}
