 ...
 * y define consultas específicas de
 * dominio utilizando tanto JPQL (Java Persistence Query Language) como métodos
...
public interface ModuloRepository extends JpaRepository<Modulo, Long> {
     ...
     * (nombreId, nombre, url, icon) para
     * optimizar la lectura de datos. Los resultados se ordenan de forma descendente
     * por el identificador del módulo.
     * </p>
     *
     ...
     * <code>mod_nombre_id</code>, permitiendo la
     * recuperación masiva de registros basada en sus identificadores de frontend
     * (DOM IDs).
     * </p>
     *
     ...
     * <code>SELECT 1...</code>) que comprueba la
     * presencia del dato sin necesidad de hidratar la entidad completa en memoria.
     * Útil para validaciones previas a la
     ...
     * integridad durante los procesos de
     * <strong>actualización</strong>. Permite comprobar que el nuevo nombre
     * asignado no entre en conflicto con otros
     * registros existentes en la base de datos, ignorando el propio registro que
     ...
     *               debe ser omitido en la búsqueda para
     ...
     * <p>
     * Ejecuta una consulta de proyección JPQL que selecciona campos específicos y
     * resuelve las relaciones (como nombres
     * de estado, subsistema y tipos) directamente en la base de datos. Los
     * resultados son mapeados automáticamente al
     * constructor del record {@link ModuloResponse}.
     * </p>
     * <p>
     * Esta estrategia evita la carga de las entidades completas ({@link Modulo}) y
     * sus asociaciones perezosas,
     * mejorando significativamente el rendimiento en listados masivos o reportes de
     * interfaz de usuario.
     * </p>
     *
     * @param pageable objeto de la interfaz {@link Pageable} que encapsula la
     *                 información de paginación (número de
     *                 página, tamaño) y criterios de ordenamiento solicitados por
     *                 el cliente.
     * @return una página ({@link Page}) conteniendo los objetos
     *         {@link ModuloResponse} proyectados. Nunca retorna
    ...
    /**
     * Recupera los datos puntuales de un módulo específico utilizando una
     * proyección de lectura optimizada.
     * <p>
     * Ejecuta una consulta JPQL explícita para seleccionar únicamente los atributos
     * definidos en el registro
     * {@link ModuloDetailResponse}. Esta operación está diseñada específicamente
     * para soportar procesos de edición
     * o auditoría, ya que recupera los identificadores (IDs) de las relaciones
     * (Estado, Subsistema, Tipos)
     * en lugar de sus descripciones, facilitando el enlace de datos en formularios
     * del cliente.
     * </p>
     *
     * @param id identificador único (llave primaria) del módulo que se desea
     *           consultar.
     * @return un contenedor {@link Optional} que incluye la proyección detallada si
     *         el registro existe,
     *         o un {@link Optional#empty()} si no se encuentra ningún módulo
     *         asociado al <code>id</code> provisto.
