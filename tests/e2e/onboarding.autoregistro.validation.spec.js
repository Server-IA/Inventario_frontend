...
//   test('ONBAV-02: endpoint de verificación rechaza token inválido', async ({ request }) => {
...
//   test.describe('Validaciones de formularios onboarding', () => {
//     test('ONBAV-03: formulario persona valida obligatorios e integridad', async ({ page }) => {
...
//       await expect(page.getByRole('heading', { name: /Formulario Persona/i })).toBeVisible();

//       await clickPrimaryButton(page, /guardar/i);
//       await expect(page.getByText(/corrige|obligatorio/i).first()).toBeVisible();
...
//       await clickPrimaryButton(page, /guardar/i);
//       await expect(page.getByText(/correo|identificaci|fecha|formato|sólo números|solo numeros/i).first()).toBeVisible();
//     });

//     test('ONBAV-04: formulario empresa valida obligatorios y NIT/correo/celular', async ({ page }) => {
//       test.skip(!E2E_BASE_URL, 'Configura E2E_BASE_URL para pruebas UI de onboarding.');
//       test.skip(
...
//       await expect(page.getByRole('heading', { name: /Formulario Empresa/i })).toBeVisible();
...
//       await clickPrimaryButton(page, /Guardar Empresa/i);
//       await expect(page.getByText(/letras|correo|sólo números|solo numeros|NIT inválido|NIT invalido/i).first()).toBeVisible();
//     });
//   });

//   test('ONBAV-05: registro bloquea contraseña demasiado débil', async ({ page }) => {
//     test.skip(!E2E_BASE_URL, 'Configura E2E_BASE_URL para validaciones de registro UI.');
